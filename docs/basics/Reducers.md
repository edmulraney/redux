...
We have two more actions to handle! Just like we did with `VISIBILITY_FILTER_CLICKED`, we'll import the `TODO_SUBMITTED` and `TOGGLE_TODO_CLICKED` actions and then extend our reducer to handle `TODO_SUBMITTED`.
...
Finally, the implementation of the `TOGGLE_TODO_CLICKED` handler shouldn't come as a complete surprise:

```js
case TOGGLE_TODO_CLICKED:
  return Object.assign({}, state, {
